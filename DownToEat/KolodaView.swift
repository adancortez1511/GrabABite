...
public class KolodaView: UIView, DraggableCardDelegate {
    ...
    private func swipedAction(direction: SwipeResultDirection) {
        animating = true
		let directionString = SwipeResultDirection.Right
		
		if (directionString == direction)
		{
			config.testImg = UIImageView(frame: CGRectMake(50, 6, 68, 68))

			UIGraphicsBeginImageContext(CGSizeMake(345, 400))
			visibleCards[0].contentView!.layer.renderInContext(UIGraphicsGetCurrentContext()!)
			let image = UIGraphicsGetImageFromCurrentImageContext()
			UIGraphicsEndImageContext()
			
			config.testImg.image = UIImage(CGImage: image.CGImage!)
			config.eventDictonary[config.eventsAttending] = config.testImg
